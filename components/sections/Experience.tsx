...
const items = [
  ...
  },
  {
    ...
    time: "2019 – 2023",
    points: ["CGPA: 3.28 / 4.00. Strong analytical foundation in philosophy and physics."]
  }
];
...
export default function Experience() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo(".exp-item", { x: -50, opacity: 0 }, { x: 0, opacity: 1, duration: 0.9, stagger: 0.2, ease: "power2.out", immediateRender: false, scrollTrigger: { trigger: ref.current, start: "top 90%", once: true } });
      gsap.fromTo(".exp-line", { scaleY: 0 }, { scaleY: 1, transformOrigin: "top", duration: 1.5, immediateRender: false, scrollTrigger: { trigger: ref.current, start: "top 90%", once: true } });
    }, ref);
    return () => ctx.revert();
  }, []);

  return (
    <section id="experience" ref={ref} className="relative mx-auto max-w-7xl px-6 py-20 md:py-28">
      <p className="mb-3 text-sm uppercase tracking-widest text-accent">Journey</p>
        ...
        <div className="space-y-12">
          {items.map((it) => (
            <div key={it.role} className="exp-item relative">
              ...
              <div className="text-xs uppercase tracking-widest text-muted">{it.time}</div>
